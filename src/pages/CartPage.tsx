                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-sm text-slate-500">{item.stall.name}</p>
                      </div>
                      <Button 
                        variant="ghost" 
                        size="icon" 
                    ...
                    <div className="flex justify-between items-center mt-2">
                      ...
                      <div className="text-right">
                        <p className="text-xs text-slate-400">R$ {item.price.toFixed(2)} / {item.product.unit}</p>
                ...
                <div className="h-48 md:h-full min-h-[180px] rounded-2xl overflow-hidden border border-slate-100 relative bg-slate-100">
                  {address ? (
                    <iframe
                      ...
                      frameBorder="0"
                      style={{ border: 0 }}
                      src={`https://www.google.com/maps/embed/v1/place?key=REPLACE_WITH_REAL_KEY_OR_USE_OSM&q=${address.logradouro},${address.localidade},${address.uf}`}
                      // Note: Google Maps Embed API requires a key. For demo, we can use a generic search URL or OpenStreetMap.
                      // Let's use a simpler approach that doesn't strictly require a key for basic display if possible, 
                      // or just a placeholder that looks real.
                      // Actually, let's use OpenStreetMap iframe which is free.
                      srcDoc={`
                        ...
                        <iframe 
                          ...
                          marginheight="0" 
                          marginwidth="0" 
                          src="https://www.openstreetmap.org/export/embed.html?bbox=${address.lon ? address.lon-0.01 : -46.6388-0.01}%2C${address.lat ? address.lat-0.01 : -23.5489-0.01}%2C${address.lon ? address.lon+0.01 : -46.6388+0.01}%2C${address.lat ? address.lat+0.01 : -23.5489+0.01}&amp;layer=mapnik&amp;marker=${address.lat || -23.5489}%2C${address.lon || -46.6388}">
                        </iframe>
                      `}
                    />
                  ) : (
